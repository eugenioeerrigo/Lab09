...
public class BordersDAO {
	...
	public List<Border> getCountryPairs(int anno, CountryIdMap map) {
		...
		try {
			...
			while (rs.next()) {
				...
				Border b = new Border(cont, map.get(rs.getInt("state1no")), map.get(rs.getInt("state2no")), rs.getInt("year"));
				result.add(b);
				cont++;
			}
